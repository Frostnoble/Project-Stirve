...
if(xTarg == x and yTarg == y and !attacking){
	...
	if(point_distance(x,y,objPlayer.x,objPlayer.y) < 164)
	{
		if(HP != MHP)
		{
...
deerinFront_x = round((x+lengthdir_x(TILE_W*spd,move_dir)) / 16) * 16;
deerinFront_y = round((y+lengthdir_y(TILE_H*spd,move_dir)) / 16) * 16;
...
if(place_meeting(xTarg,yTarg,objPlayer) && !stunned  && HP != 5){
	...
	if(!global.inv)
	{
		...
		if(global.HEALTH - 30 < 0){global.HEALTH = 0}else{global.HEALTH -= 30}
...
var _inst = instance_place(deerinFront_x, deerinFront_y, objGrass);
