...
if(global.gamemode == 4 && global.inFront_x == x && global.inFront_y == y){
	...
	if(modetype)
	{
		...
		if((keyboard_check_pressed(A_Key) || keyboard_check_pressed(A_Key2)) && (ds_list_size(storage) < 9 || checkInventory(storage_selector,storage)) && ds_list_size(storage) >= 0 && ds_list_size(objInventory.inventory) > 0)
		{
